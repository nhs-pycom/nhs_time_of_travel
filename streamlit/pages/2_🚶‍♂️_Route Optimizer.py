...
with st.form("MSR_inputs"):
    ...
    new_start_address = st.text_input(
        "Enter a new start address (if not in the list above):"
    )
    if new_start_address:
        ...
        start_address = st.selectbox(
            "Select start address from the dataframe above",
            options=start_address_options,
            index=len(start_address_options) - 1,
        )
    else:
        start_address = st.selectbox(
            "Select start address from the dataframe above",
    ...
    radius_miles = st.number_input(
        "Enter radius in miles", min_value=1, max_value=5, value=1
    )
